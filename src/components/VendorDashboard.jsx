...
const VendorDashboard = ({ user }) => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        if (shopData._id) {
          const ordersRes = await fetch(`http://localhost:5001/api/orders/vendor/${shopData._id}`);
          if (ordersRes.ok) {
  ...
  const toggleShopStatus = async () => {
    try {
      const response = await fetch(`http://localhost:5001/api/vendors/${shop._id}/toggle-status`, { method: 'PUT' });
      if (response.ok) {
                ...
                <div className="bg-slate-50 p-4 rounded-xl mb-6 border border-slate-100">
                  ...
                  {order.paymentMethod === 'PromptPay' && order.paymentSlip && (
                    <div className="mt-3">
                      ...
                      <a href={`http://localhost:5001${order.paymentSlip}`} target="_blank" rel="noreferrer">
                        <img src={`http://localhost:5001${order.paymentSlip}`} alt="Slip" className="w-full h-32 object-cover rounded-lg border border-slate-200 hover:opacity-80 transition-opacity cursor-pointer" />
                      </a>
                    </div>
                  )}
                </div>
